...
beforeEach(async () => {
  ...
  await db.run(`UPDATE portfolios SET kind = 'paper', opening_cash = '10000' WHERE id = ?`, [
    TEST_PORTFOLIO_ID,
  ]);
});
...
describe("paper trading", () => {
  it("starts with the opening balance and nothing held", async () => {
    const state = await paperState(db, paper);
  ...
  it("spends cash on a buy and returns it on a sell", async () => {
    await placePaperTrade(db, paper, { side: "buy", symbol: "NVDA", quantity: 10 },
      quote("NVDA", 200), NOW);
    expect((await paperState(db, paper)).cash.toFixed(2)).toBe("8000.00");

    await placePaperTrade(db, paper, { side: "sell", symbol: "NVDA", quantity: 4 },
      quote("NVDA", 250), NOW);
    expect((await paperState(db, paper)).cash.toFixed(2)).toBe("9000.00");

    const held = (await paperState(db, paper)).holdings.get("NVDA");
    expect(held?.quantity.toNumber()).toBe(6);
  });

  it("refuses to spend money that is not there", async () => {
    await expect(
      placePaperTrade(db, paper, { side: "buy", symbol: "NVDA", quantity: 100 },
        quote("NVDA", 200), NOW),
    ).rejects.toBeInstanceOf(PaperTradeError);

    expect((await paperState(db, paper)).cash.toFixed(2)).toBe("10000.00");
  });
  ...
  it("refuses to sell more than is held", async () => {
    await placePaperTrade(db, paper, { side: "buy", symbol: "NVDA", quantity: 2 },
      quote("NVDA", 200), NOW);

    await expect(
      placePaperTrade(db, paper, { side: "sell", symbol: "NVDA", quantity: 3 },
        quote("NVDA", 200), NOW),
  ...
  it("refuses a stale price", async () => {
    await expect(
      placePaperTrade(db, paper, { side: "buy", symbol: "NVDA", quantity: 1 },
  ...
  it("refuses a fractional or negative quantity", async () => {
    for (const quantity of [0, -5, 1.5]) {
      await expect(
        placePaperTrade(db, paper, { side: "buy", symbol: "NVDA", quantity },
          quote("NVDA", 200), NOW),
...
describe("paper holdings in the shared positions table", () => {
  ...
  it("replaces rather than accumulates on a second sync", async () => {
    await placePaperTrade(db, paper, { side: "buy", symbol: "NVDA", quantity: 1 },
      quote("NVDA", 200), NOW);

    await syncPaperPositions(db, paper, new Map([["NVDA", quote("NVDA", 200)]]), NOW);
    await syncPaperPositions(db, paper, new Map([["NVDA", quote("NVDA", 210)]]), NOW);
